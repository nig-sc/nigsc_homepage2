...
### ストレージ環境
Parabricks の実行には，十分なサイズのデータセットを格納でき，高速に読み書きできるストレージ環境の確保は不可欠である. 遺伝研 igt および高火力 PHY は，高速な NVMe SSD をローカルストレージとして備えている事から，各アプリケーションの入出力用のストレージ領域として利用した. 検証に利用した遺伝研 igt と高火力 PHY の搭載されているストレージ構成と，fio コマンドによるファイルのシーケンシャルな READ/WRITE による I/O 性能測定結果を[表2](#表-2-検証環境のストレージ構成とシーケンシャル-i/o-性能)に示す. 測定対象にある/tmp は，システムディスク上の NVMe SSD を指している. 遺伝研 igt では/data 領域での性能に差は少なかった.一方で，高火力 PHY では約 7 倍の性能差があった.
...
### ノードおよびバージョンごとの解析時間
[表3](#表-3-ノード及び-parabricks-バージョンごとの解析時間)にノードおよび Parabricks のバージョンごとの解析時間を示す.
