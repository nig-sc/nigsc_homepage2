...
## 1. テストデータの準備 {#prepare-test-data}

**この作業は、手元のPCで行います**

チュートリアルをすすめるにあたって、使用するテストデータをダウンロードし、遺伝研スパコン個人ゲノム解析区画へコピーします。

### テスト用データのダウンロード {#download-the-test-data}
...
### 遺伝研スパコン個人ゲノム解析区画へのコピー {#copy-to-personal-genome-analysis}
...
## Imputation Workflow用の設定ファイルの生成 {#generation-file}
...
## Imputation Workflowの実行 {#execute-imputation-workflow}
...
### インピューテーション後のデータ {#data-after-imputation}
...
### インピュテーション後のデータを手元のPCへ持ってくる {#download-data-to-local-computer}

**この作業は、手元のPCで行います**

#### Guacamole内のブラウザでファイルをダウンロードしたところからの取得 {#get-file-from-guacamole}
...
#### sapporo-serviceの結果ディレクトリからの取得 {#get-from-result-directory-sapporo-service}
